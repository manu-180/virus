...
type AnyDb = ReturnType<typeof createAdminClient> & { from: (t: string) => any };
...
async function sumField(
  ...
  if (error) throw new Error(`quota query failed: ${(error as { message: string }).message}`);
...
async function countRows(userId: string, service: string, sinceMs: number): Promise<number> {
  ...
  const { count, error } = await db
    ...
    .eq('service', service)
    .gte('created_at', since);
  if (error) throw new Error(`quota count failed: ${(error as { message: string }).message}`);
...
export async function recordUsage(input: {
  ...
  if (error) throw new Error(`recordUsage failed: ${(error as { message: string }).message}`);
}
